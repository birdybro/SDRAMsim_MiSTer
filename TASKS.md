...
- [ ] **WRITE auto-precharge interacts badly with the per-data-cycle `last_write[bank]` stamp.** `consume_write_data` at burst end stamps `last_write[bank] = $realtime` and then calls `maybe_auto_precharge` → `do_precharge`, which trips its own `tWR` check (delta=0 < `tWR_MIN`). Real silicon handles tWR internally for WRITE+AP — the chip schedules the implied PRECHARGE `tWR` after the last data cycle, and the controller doesn't need to wait. Fix: when AP fires from a write burst, skip the tWR check inside `do_precharge` AND stamp `last_precharge[bank] = $realtime + tWR_MIN` so the subsequent tRP check on the next ACT counts from the *real* (deferred) precharge time, not the AP-issue time. MiSTer corpus uses explicit PRE much more than WRITE+AP so impact is narrow, but it's a real bug for any controller that does use it.
