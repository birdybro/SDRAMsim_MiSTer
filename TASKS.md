...
- [~] **Bring-up target order** (agreed): (1) ~~`MemTest_MiSTer`~~ **landed** as `xsds_tb_memtest.sv` (`make -C verilator memtest`). 50 µs of activity post-rst_n: both chips init clean, BL=4 CL=3 bursts run, distributed refresh ticking — 0 errors / 0 warnings. (1b) ~~`NeoGeo_MiSTer`~~ **landed** as `xsds_tb_neogeo.sv` (`make -C verilator neogeo`). Pulled the controller from upstream into `ref/MiSTer SDRAM Controller Modules/NeoGeo_MiSTer/`. Init sequence validates both chips at BL=4 CL=2 single-write-burst, controller asserts `ready` after the 121 µs internal startup, 2 refreshes queued per chip, zero violations. Notable findings: like MemTest, NeoGeo is XSDS-native and bypasses the adapter (drives `SDRAM_nCS` from `addr[26]` and `chip` register). Its internal startup is only 121 µs (less than the chip's 200 µs power-up), so the TB holds `init=1` for 80 µs before release so startup completes at ~201 µs. (2) A representative BL=1 CL=2 console core (`NES_MiSTer` or `Genesis_MiSTer`). (3) A full-page-burst core (Saturn/MegaCD). (4) `jtframe_sdram` as its own track. Notable findings during MemTest and NeoGeo bring-up: both use `altddio_out` (stubbed in `verilator/altddio_out_stub.sv`); both have incomplete reset of the command pipeline (TB gates RAS/CAS/WE to NOP during reset/init so the chip's 200 µs power-up check sees clean idle). Note: `Jaguar_MiSTer` / `Arcade-Darius_MiSTer` self-refresh paths can't be exercised through the XSDS connector since CKE is tied high — they're chip-level standalone tests, not XSDS regressions.
