...
- Verilator coverage: `make -C verilator lint` lints the model + wrapper stub; `make -C verilator smoke` runs the synthetic adapter+chip stim TB; `make -C verilator memtest` / `neogeo` / `nes` / `saturn` / `psx` build and run the corresponding bring-up TBs. All six currently pass clean (PSX emits 2 expected tREFI-interval warnings — see its TB comments).
- PSX_MiSTer required a sim-only patched copy of its controller at `verilator/psx_sdram_for_verilator.sv` to work around three Verilator-with-`--bbox-unsup`-and-`--timing` quirks: (a) `command <= 'Z` in the `!SDRAM_EN` else branch silently breaks `command`'s NBA-override path (the override line `command <= CMD_PRECHARGE/...` gets dropped, chip only sees NOPs); (b) `inout reg [15:0] SDRAM_DQ` with multi-driver wire-OR semantics merges back-to-back BL=2 write data via OR (`0xBABE | 0xCAFE = 0xfafe` stuck on the wire), so we split into separate `SDRAM_DQ_OUT/OE/IN` ports and re-merge in the TB; (c) the chip model's NBA-registered `dq_out` arrives 1 cycle later on the wire than real-silicon combinational output, so PSX's ch3 read-pipeline capture indices are shifted from `[7,6,2]` to `[6,5,1]` and the chip instances are given `tLZ_MIN=0.0` / `tAC_MAX=0.0` to make the OE/data NBAs commit at end-of-timestep. None of this reflects real PSX behavior — these are sim artifacts of Verilator's tristate / inout handling. Commercial sims and real silicon don't need any of it. Verilator needs `--timing` for the NBA intra-assignment delays to model `tAC` / `tHZ` / `tLZ`, and needs `--bbox-unsup` because Verilator does not support inout pass-through through hierarchy. Both flags are wired up in `verilator/Makefile`.
